...
const SearcheableCitiesList = () => {
  ...
  return (
    ...
    <div className="relative mb-4 rounded-md shadow-sm">
        ...
        {cities.map((city, i) => (
          <Transition as="div" enterFrom="opacity-0 translate-y-4" enter="transition-all ease-out duration-200" leaveFrom="opacity-100 translate-y-0" leave="transition-all ease-in duration-200" show={results.has(city.link)} key={city.link}>
                <CityCard city={city} className={i % 2 ?'rotate-1' :'-rotate-1' }/>
            </Transition>
        ))}
      </div>
...
const EmptyState = () => {
  return (
    <div className="w-full rounded bg-indigo-700 px-12 py-6 text-white">
        ...
        <p>
        Want to play in your city? Contribute on{' '}
      <a className="font-medium hover:underline" href="https://github.com/atreyut/brt-memory/">
        Github
      </a>
      .
    </p>
...
const SuggestCity = () => {
  return (
    <p className="mt-6">
      If you want the game to be available in your city, contribute on{' '}
      <a className="font-medium hover:underline" href="https://github.com/atreyut/brt-memory/">
        Github
      </a>
      .
